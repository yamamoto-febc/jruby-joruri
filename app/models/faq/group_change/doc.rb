# encoding: utf-8
class Faq::GroupChange::Doc < Sys::GroupChangeItem

  belongs_to :entity,   :foreign_key => :item_id,      :class_name => 'Faq::Doc'
  ...
  def pull(change, setting)
    ...
    item.or 'question', 'LIKE', "%#{change.old_name}%" if t.include?('question')
    if t.include?('body')
    ...
    self.class.find(:all, :conditions => { :model => entity_cls_name } , :order => 'id').each do |temp|
      if org = temp.entity
        changes.each do |change|
          ...
          org.question = org.question.gsub(/#{change.old_name}/, change.name) if org.question && t.include?('question')
